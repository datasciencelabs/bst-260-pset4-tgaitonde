...
population <- population |> ## Use janitor row to names function
  ...
  pivot_longer(-"state_name", names_to = "Year", values_to = "Population") |> # use pivot_longer to combine years into 2 column
  mutate(Year=gsub("^POP_", "", Year)) |>
  mutate(Population = as.numeric(Population), Year = as.numeric(Year)) |>   # parse all relevant columns to numeric
...
cases |> 
  ...
  #inner_join() or left_join and filter by na
  cases_per_100k = calc |>
  ggplot() +
  geom_line() +
  labs +
...
#death |> filter() |> group_by()
#|> summarize(), slice_max() |>
 # mutate(reorder states) |>
  #ggplot()
