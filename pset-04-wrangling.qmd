...
resp_content_type(response) #function finds the content type of variable response
...
```{r}
population <- resp_body_json(response, simplifyVector = TRUE) #assign variable population to change data into matrix
...
population <- population |> ## Use janitor row to names function
  ...
  pivot_longer(cols = starts_with("POP"), 
               names_to = "year", 
               values_to = "population") |> # use pivot_longer to combine years into 2 column
...
```{r}
population |> 
  ggplot(aes(y=reorder(state_name, population), x = population)) + #create barplot
  geom_col() +
  labs(x = "Population", 
      y = "States",
...
```{r}
url8 <- "https://github.com/datasciencelabs/2025/raw/refs/heads/main/data/regions.json"
```
...
regions_raw <- fromJSON(url8) #parses json file into r

regions_raw$region_name[regions_raw$region_name == "New York and New Jersey, Puerto Rico, Virgin Islands"] = "NY NJ PR VI" #shortens region name
...
regions <- regions_raw |>
  ...
  mutate(
    state_name = str_remove_all(state_name, 'c\\("|"\\)|"'),
  )
...
check_cases_raw <- request(api) |> 
  req_url_query() |>
  req_perform() |> 
  resp_status()
  
#no the data is not all here, likely due to a limit on the number of rows from the CDC
```
...
#filter by NA
#make sure right type, date shouldn't be a char
```
...
cases_filtered <- cases_raw |> 
  ...
  left_join(population, by = "state") |>   #inner_join() or left_join and filter by na
  filter(!is.na(state), !is.na(date), !is.na(cases), !is.na(region_name)) |>
  filter(year(date) == Year)
  
ggplot(cases_filtered, aes(x = date, y = 100000*(cases/population), col=state)) +
  geom_line() +
  labs(x= "Time",
       y = "Cases Per 100,000 People",
       title = "Cases Per 100k in 2020-2021") +
...
cases_filtered |>
  filter(year(date) == 2020 | year(date) == 2021) |>
  #filter(state %in% population$state) |> #this isn't filtering out USA and NYC
  mutate(
    year = year(date),
...
deaths <- request(deaths_url) |> 
  ...
  filter(!is.na(state), !is.na(end_date), !is.na(covid_19_deaths)) |>
  filter(state %in% population$state_name) |> #check this
  mutate(
    date = ymd_hms(end_date),
...
check_deaths <- request(deaths_url) |> 
  req_url_query() |>
  req_perform() |> 
  resp_status()
...
deaths |> 
  ...
  ggplot(aes(x = deaths, y = state)) +
    geom_col(fill = "darkgreen") +
   labs (x = "States",
         y = "Total Deaths By Covid",
         title = "Highest 10 States in Deaths by Covid From Jan 2020 - Sept 2023")
